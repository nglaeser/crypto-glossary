...
!!! example "ECDSA signatures"
    === "Construction (additive notation)"
...
!!! example "Schnorr signatures"
    ...
    === "Notes"
        Schnorr signatures can be viewed as a [NIZK](../Areas-of-Cryptography/zk.md#nizk) [PoK](../Areas-of-Cryptography/zk.md#pok) of the discrete logarithm of the public key (i.e., knowledge of the secret key). Note that the Schnorr signing algorithm is essentially a standard [sigma-protocol for DLog](../Areas-of-Cryptography/zk.md#sigma-dlog), made non-interactive with the [Fiat-Shamir transform](../techniques.md#fiat-shamir).

<a name="eddsa"></a>
!!! example "EdDSA"
    EdDSA is a variant of the [Schnorr signature scheme](#schnorr-sig) for twisted Edwards [elliptic curves](../Areas-of-Cryptography/ecc.md).

<a name="bls-sig"></a>
!!! example "BLS signatures [[BLS01]](https://hovav.net/ucsd/dist/sigs.pdf)"
    === "Construction (additive notation)" 
...
**Group signatures** { #group-sig }
: One person in a group can produce a signature on behalf of everyone in the group. The signature does not reveal which member of the group produced it, so the signer benefits from some k-anonymity. Unlike [ring signatures](#ring-sig), there is also a special party called the **group manager** who is able to determine the signer given the signature.
