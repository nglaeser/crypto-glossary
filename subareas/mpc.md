...
Schemes:
* **Exclusive OR (XOR) secret-sharing**: For a secret _s_, set Party _i'_s share to some random value <img alt="r_i" src="https://render.githubusercontent.com/render/math?math=r_i" style="transform: translateY(20%);" />, except for a designated party which gets <img alt="s~\text{XOR}~r_1~\text{XOR}~\ldots~\text{XOR}~r_N" src="https://render.githubusercontent.com/render/math?math=s~%5Ctext%7BXOR%7D~r_1~%5Ctext%7BXOR%7D~%5Cldots~%5Ctext%7BXOR%7D~r_N" style="transform: translateY(20%);" />. The shares XOR together to _s_, but each individual share looks random.
